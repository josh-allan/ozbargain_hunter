...
def search_feed(feed_url, interesting_searches, last_updated, matched_ids):
    ...
    for entry in feed.entries:
        ...
        if any(
            term in entry.title or term in entry.description
            for term in interesting_searches
        ):
    ...
    return (
        matching_entries,
        feed.entries[-1].published_parsed if feed.entries else None,
        new_matched_ids,
    )


while True:
    matching_entries, last_updated, new_matched_ids = search_feed(
        feed[0], interesting_searches, last_updated, matched_ids
    )
    if matching_entries:
        for entry in matching_entries:
            print(
                f"Matching entry found in {feed[0]}: {entry.title} at {now.strftime('%d/%m/%Y, %H:%M:%S')}"
            )
            # Notify via telegram API:
            message = f"A new matching entry was found in {feed[0]}: {entry.title}\n{entry.link} at {now.strftime('%d/%m/%Y, %H:%M:%S')}"
